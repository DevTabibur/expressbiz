    ...
    <div>
      <div className="container mx-auto px-4">
        <div className="grid md:grid-cols-1 text-center">
          <h1 className="text-3xl font-bold  text-accent mb-4">Welcome to {title}</h1>
          <figure>
            <img className="w-full h-96 rounded shadow-2xl " src={`http://localhost:5000/${image}`} alt="service_image" />
          </figure>
          <p className="mt-8  text-info">{description}</p>
        </div>
      </div>
    </div>
    </div>
